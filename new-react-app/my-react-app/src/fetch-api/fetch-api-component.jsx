  ...
  return (
    <div>
      ...
      {products.length === 0 ? <p>No products</p> : products.map(item => (
        <div key={item.id}><h3>{item.title}</h3><p>${item.price}</p></div>
      ))}
      
      <h2>Todos</h2>
      {todos.length === 0 ? <p>No todos</p> : todos.map(item => (
        <div key={item.id}><p>{item.todo}</p></div>
      ))}
      
      <h2>Users</h2>
      {users.length === 0 ? <p>No users</p> : users.map(item => (
        <div key={item.id}><h3>{item.firstName} {item.lastName}</h3><p>{item.email}</p></div>
      ))}
    </div>
  );
};
